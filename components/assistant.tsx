...
export function Assistant({ locale, copy }: { locale: Locale; copy: SiteCopy }) {
  ...
  const [turns, setTurns] = useState<Turn[]>([{ role: "bot", text: copy.assistantIntro }]);
  ...
  return (
    <>
      <button className="btn btn-primary assistant-launch" type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)}>
